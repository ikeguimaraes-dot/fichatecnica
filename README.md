...
### Sincronização diária, manual e leitura rápida
...
O cron `le-chef-everest-daily-sync` solicita a atualização de todas as unidades todos os dias às **01:00 de Brasília** (`0 4 * * *` em GMT). Configuração reproduzível em `supabase/operations/schedule_everest_daily.sql`. O solicitante é nulo para identificar a automação. Se já existe trabalho ativo, ele é preservado sem criar concorrência; falhas anteriores do mesmo escopo são retomadas. O horário marca o início: a conclusão depende do volume e da disponibilidade da API. O cron `le-chef-everest-worker` verifica a cada minuto se existe trabalho solicitado pelo agendamento ou pelo botão e aciona `/api/everest-worker` por `pg_net`. Ele não consulta o Everest quando não há trabalho. O token fica no Vault sob `lechef_everest_worker_token`, com o mesmo valor secreto na Vercel. O processador usa blocos de aproximadamente 45 segundos, checkpoints por etapa e lease de 150 segundos para evitar concorrência e retomar após interrupções. A origem tem intervalo mínimo de 1,2 segundo; 412/429 têm nova tentativa. Cinco falhas consecutivas na mesma etapa encerram o trabalho com erro, preservando versões publicadas. Solicitar novamente o mesmo escopo retoma o último trabalho falho. A fila aceita apenas um trabalho ativo, devido ao limite global da API.
